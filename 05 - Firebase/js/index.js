$(function () {
    ...
    var configurarBotao = function (idBotao, idToggle, idIcon, flag) {
        ...
        idBotao.click(function (e) {
            ...
            if (jsonState.state == 'on') {
                newLightState = {
                    state : 'off',
                    date : upDate.toISOString()
                }
                updateFirebase(idState, newLightState);
                sessionStorage.setItem(idState, JSON.stringify(newLightState));
            }
            else {
                newLightState = {
                    state : 'on',
    ...
    function configuraStates(idButton) {
        ...
        lightState = sessionStorage.getItem(idButton)
        var dateLS = new Date();
        if (lightState) {
            var ls = JSON.parse(lightState);
            if (ls.state == 'on') {

                newLightState = {
                    state : 'init',
                    date : dateLS.toISOString()
                }
                sessionStorage.setItem(idButton, JSON.stringify(newLightState));
                updateFirebase(idButton, newLightState);
                $("#" + idButton).click();
            }
        }
        else {
    ...
    function updateFirebase (id, updateData)
    {
        $.ajax({
            url: 'https://cad2122-default-rtdb.firebaseio.com/05-APIS/2202578/' + id + '.json',
            type: 'PUT',
            data: JSON.stringify(updateData),
            success: function(result) {
                console.log('Sucedeed to update Firebase !!');
            }
        });  
    }

    var getItem = getItemFirebase('kitchenLights', 'state');

    function getItemFirebase (id,item)
    {
        var uData;
        $.ajax({
            url: 'https://cad2122-default-rtdb.firebaseio.com/05-APIS/2202578/'+ id + '.json',
    ...
    {
        /* Adicionar aqui a jsonState e lightState*/ 
    }
